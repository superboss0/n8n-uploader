...
REQ_PATTERNS = [
    ...
    r"\b(подтвердите|подтверди|подтвердить|проверьте|проверь|проверить)\b.*\b(актуальност[ьи]|актуален|валидност[ьи]|валиден)\b.*\bкошел[её]к\b",

    # "Просьба подтвердить актуальность кошелька ..."
    r"\b(просьба|прошу)\b.*\b(подтвердить|проверить)\b.*\bкошел[её]к\b",

    # более короткие варианты (на всякий)
